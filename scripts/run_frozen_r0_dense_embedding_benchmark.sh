...
RUNS_ROOT="${RUNS_ROOT:-$REPO_ROOT/runs/dense_event_reid_embedding_frozen/$INITIALIZATION}"
RESULTS_ROOT="${RESULTS_ROOT:-$REPO_ROOT/results/dsec_mot_dense_embedding_frozen/$INITIALIZATION}"
LOG_DIR="$RUNS_ROOT/logs"
LOG_PATH="$LOG_DIR/full_dense_frozen_r0_${INITIALIZATION}_benchmark.log"
...
required_checkpoints=("$ALL_R0" "$CAR_R0")
if [[ "$INITIALIZATION" == "warmstart" ]]; then
  required_checkpoints+=("$D1_INITIAL_HEAD" "$D2_INITIAL_HEAD")
fi
for checkpoint in "${required_checkpoints[@]}"; do
  if [[ ! -f "$checkpoint" ]]; then
    echo "Nie znaleziono wymaganego checkpointu: $checkpoint" >&2
    exit 1
  fi
done
...
run_name_for() {
  local variant="$1"
  if [[ "$variant" == "D1" ]]; then
    printf 'event_frame_voxel_grid_bins3_w32_gated_two_branch_dense_d1_frozen_r0_%s' "$INITIALIZATION"
  else
    printf 'event_frame_voxel_grid_bins3_w32_gated_two_branch_dense_d2_frozen_r0_recurrent_%s' "$INITIALIZATION"
  fi
}
...
train_variant() {
  ...
  local initial_embedding_checkpoint="${4:-}"
  ...
  if [[ -n "$initial_embedding_checkpoint" ]]; then
  ...
  echo
  echo "Trening dense $scope $variant z zamrożonego R0, inicjalizacja=$INITIALIZATION"
  run_command "${command[@]}"
}
...
run_evaluation() {
  ...
  local command=(
    ...
    --tracker-name "dense_frozen_r0_${mode}"
...
select_detector_score() {
  ...
  for score in "${scores[@]}"; do
    run_evaluation "$scope" D1 detector_score motion val "$checkpoint" "$score" 0.25 0.50
  done

  local best_line
  best_line="$(best_summary_from_pattern "$RESULTS_ROOT/${scope}_D1_detector_score_motion_val_score*_appearance025_proximity050")"
...
for scope in all_classes car_only; do
  ...
  if [[ "$INITIALIZATION" == "warmstart" ]]; then
    train_variant "$scope" D1 "$r0_checkpoint" "$D1_INITIAL_HEAD"
    train_variant "$scope" D2 "$r0_checkpoint" "$D2_INITIAL_HEAD"
  else
    train_variant "$scope" D1 "$r0_checkpoint"
    train_variant "$scope" D2 "$r0_checkpoint"
  fi
done

if [[ "$DRY_RUN" == "1" ]]; then
  echo
  echo "Dry-run dense D1/D2 ($INITIALIZATION): po treningu skrypt wybiera score threshold przez HOTA motion na walidacji, następnie best_hota.pt przez HOTA ReID każdej epoki, a na końcu appearance/proximity również przez HOTA walidacyjne. Ostatnia epoka jest dodatkowo oceniana diagnostycznie."
  exit 0
fi

for scope in all_classes car_only; do
  reference_checkpoint="$(run_dir_for "$scope" D1)/epoch_001.pt"
  select_detector_score "$scope" "$reference_checkpoint"
  selected_score="$SELECTED_SCORE"
  echo
  echo "Wybrany score threshold dla $scope: $selected_score"

  for variant in D1 D2; do
